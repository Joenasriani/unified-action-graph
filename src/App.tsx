...
import { ConnectorView } from './components/views/ConnectorView';
import { OsintView } from './components/views/OsintView';
...
export default function App() {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'connectors': return <ConnectorView />;
  ...
  return (
    <div className="grid grid-cols-[240px_1fr] grid-rows-[64px_32px_1fr] h-screen w-full bg-surface-800 text-slate-200 overflow-hidden font-sans gap-[1px]">
      <header className="col-span-2 flex items-center justify-between px-6 bg-surface-900 relative z-20">
        <div><h1 className="text-lg font-bold tracking-tighter text-cyan-400">UNIFIED ACTION GRAPH <span className="text-slate-500 font-normal ml-2 text-xs uppercase">v1.1.0-OSINT</span></h1></div>
        <div className="flex items-center gap-6 text-xs font-medium">
          ...
          <div className="flex items-center gap-2 text-slate-400">LIVE PUBLIC OSINT: DNS/RDAP/URLHAUS/GITHUB</div>
      ...
      <div className="col-span-2 row-start-2 flex items-center justify-center border-y border-amber-500/30 bg-amber-500/10 px-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-amber-200">
        Demo telemetry remains demo · OSINT enrichment uses safe public metadata routes only · no private scraping, credentials, exploit execution, malware downloads, or external actioning
      </div>
